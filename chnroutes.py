...
def generate_quagga(metric):
    results = fetch_ip_data()
    upscript_header=textwrap.dedent("""\
    """)

    upfile=open('zebra.conf','w')

    upfile.write(upscript_header)
    upfile.write('\n')

    for ip,mask,masklen in results:
        upfile.write('ip route %s/%s 192.168.253.1\n'%(ip,masklen))

    print "For quagga only."
...
if __name__=='__main__':
    ...
    elif args.platform.lower() == 'quagga':
        generate_quagga(args.metric)
    else:
